...
int main() {
  ...
  int second_last_connected = -1;
  unordered_set<int> connected_ids;
  for (auto &r : allPairs) {
    int first_position = r.idA;
    int second_position = r.idB;
    last_connected = first_position;
    second_last_connected = second_position;

    connected_ids.insert(first_position);
    connected_ids.insert(second_position);

    solution.push_back({first_position, second_position});
    if (connected_ids.size() == positions.size())
      break;
  }

  ll ans = 1LL * positions[last_connected].second[0] *
           positions[second_last_connected].second[0];
  cout << ans << '\n';
  return 0;
}
